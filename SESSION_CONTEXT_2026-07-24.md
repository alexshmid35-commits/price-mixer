...
ноутбучных проверок на `http://127.0.0.1:5001`.

По запросу пользователя в `REMAINING_WORK.md` добавлен дальнейший roadmap
функциональности, чистоты и производительности. Главный вывод: максимум после
перехода каталога на SQLite не достигнут. Каталог уже использует WAL и FTS5,
однако рабочая сессия всё ещё опирается на pandas/JSON/XLSX, серверная таблица
на каждом запросе проходит все строки, а глобальные no-cache headers запрещают
кэширование статических файлов.

Следующий локальный спринт:

1. создать воспроизводимый benchmark на 3–5, 25 и 100 тысяч строк;
2. зафиксировать baseline загрузки, таблицы, поиска, зелёного подбора и XLSX;
3. закэшировать метаданные таблицы и разрешить versioned-cache статики;
4. повторно измерить результат;
5. сделать read-only SQL-прототип `session_products` за feature flag и сверить
   его ответы с текущим JSON;
6. только после полной сверки переносить точечные изменения рабочей сессии в
   SQL и выводить XLSX из горячего пути.

PostgreSQL для текущего localhost-режима не нужен: он станет оправдан при
нескольких web-процессах/серверах, большом числе одновременных записей,
удалённом общем доступе или требованиях high availability.

Когда будет принято решение о Linux deployment, получить адрес/имя сервера,
способ SSH-доступа и реальный домен; затем проверить машину и выполнить
migration plan с реальными путями.

## Продолжение локальной миграции

В ветке `codex/migration-complete` реализован следующий безопасный этап:

- активные строки прайса индексируются в `session_products.db`;
- серверная таблица использует SQL-фильтры, стабильную сортировку, FTS5-поиск
  и заранее рассчитанные общие счётчики;
- JSON остаётся совместимым snapshot и автоматическим fallback;
- 6 реальных сценариев на 12 847 строк построчно совпали со старым движком;
- на 100 тысячах строк SQL page p95 составил 2,7 мс, FTS search p95 — 25,2 мс;
- экспериментальный подбор получил общий cache кандидатов по revision,
  журнал ручных решений и метрики качества по категориям;
- подтверждения и отказы по-прежнему изолированы по поставщику;
- повторный подбор 200 реальных товаров ускорился с 17,2 секунды до 81 мс.

Финальная автоматическая проверка этапа: 784 Python tests, полный JavaScript
syntax check, Playwright 3/3 и production smoke 4/4. Проверенный backup перед
переключением содержит 14/14 объектов и не содержит secrets.
