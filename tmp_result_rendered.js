...
function initNoIdFilterUI(){
    ...
    (function(){
        ...
        function renderIvenModal(st){
            ...
            var queuedCpu = noMatch.filter(function(m){ return String((m && m.cpu_issue) || '') === 'queued'; });
            var noModelCpu = noMatch.filter(function(m){ return String((m && m.cpu_issue) || '') === 'no_model'; });
            var noCandCpu = noMatch.filter(function(m){ return String((m && m.cpu_issue) || '') === 'no_candidates'; });

            // Update summary stats
            var el = function(id){ return document.getElementById(id); };
            if(el('iven-modal-title')) el('iven-modal-title').textContent = (st && st.report_title) || (reportMode === 'cpu' ? 'Отчёт CPU N-Tech' : 'Отчёт подбора IVEN-бридж');
            ...
            function cpuMetaHtml(item){
                if(reportMode !== 'cpu') return '';
                var parts = [];
                if(item && item.cpu_brand){
                    parts.push('Производитель: <b>' + escapeHtml(String(item.cpu_brand)) + '</b>');
                }
                if(item && item.cpu_model){
                    parts.push('Модель: <b>' + escapeHtml(String(item.cpu_model)) + '</b>');
                }
                if(item && item.cpu_issue_label){
                    parts.push('Статус: <b>' + escapeHtml(String(item.cpu_issue_label)) + '</b>');
                }
            ...
            if(withCands.length > 0){
                ...
                sepC.innerHTML = '<td colspan="5" style="padding:8px 14px;background:#eff6ff;color:#1d4ed8;font-size:12px;font-weight:700;border-top:2px solid #bfdbfe;">' +
                    (reportMode === 'cpu'
                        ? ('CPU для ручного подтверждения (' + withCands.length + '):')
                ...
                withCands.forEach(function(m, i){
                    ...
                    candsHtml += '</div>';
                    tr.innerHTML =
                        '<td colspan="2" style="padding:7px 14px;color:#1f2937;font-size:12px;max-width:280px;">' + (m.name || '') + cpuMetaHtml(m) + '</td>' +
            ...
            if(showNoMatch && withoutCands.length > 0){
                ...
                sep.innerHTML = '<td colspan="5" style="padding:8px 14px;background:#fef9c3;color:#92400e;font-size:12px;font-weight:700;border-top:2px solid #fde68a;">' +
                    (reportMode === 'cpu'
                        ? ('Проблемные CPU — без модели или без кандидатов (' + withoutCands.length + '):')
                ...
                withoutCands.forEach(function(m, i){
                    ...
                    tr.innerHTML =
                        '<td colspan="2" style="padding:7px 14px;color:#6b7280;font-size:12px;">' + (m.name || '') + cpuMetaHtml(m) + '</td>' +
